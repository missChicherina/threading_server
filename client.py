                ...
                if self.status == "auth":
                    ...
                    if user_input != "":
                        if user_input == "exit":
                            ...
                            pause_server = pickle.dumps(["resume", "Запустить прослушивание порта", self.username])
                            self.sock.send(pause_server)
                        elif user_input == "ClearLogs":
                            clear_logs = pickle.dumps(["ClearLogs", "Очистка логов", self.username])
                            self.sock.send(clear_logs)
                        elif user_input == "ClearIdentify":
                            clear_identify = pickle.dumps(["ClearIdentify", "Очистка файла идентификации", self.username])
                            self.sock.send(clear_identify)
